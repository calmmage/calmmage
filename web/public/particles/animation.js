...
const CLICK_REACTION_STRENGTH = 0.5;
const SCATTER_DURATION = 10;
const CLICK_REACTION_DISTANCE = 2.5;
const CLICK_USE_DISTANCE = false;
...
document.addEventListener('click', (event) => {
    ...
    for (let i = 0; i < SCATTER_DURATION; i++) {
        let click_reaction_scale = 1 - i / SCATTER_DURATION;
        let strength = CLICK_REACTION_STRENGTH * click_reaction_scale;
        setTimeout(() => {
            scatterCubes(cubes, mouseX, mouseY, strength, CLICK_REACTION_DISTANCE, CLICK_USE_DISTANCE);
        }, i*10);
    }
    setTimeout(() => {
        FOLLOW_USE_DISTANCE = true;
    }, SCATTER_DURATION*10 * 10);
