...
def autocast_args(you_forgot_brackets=None, fail_on_errors=True, cast_none=False):
    ...
    def cast_arg(arg, arg_sig, func_name):
        try:
            ...
            if fail_on_errors:
                raise TypeError(
                    f"Failed to cast argument {arg_sig.name} of funciton {func_name} "
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            casted_args = []
            args = [
                cast_arg(arg, arg_sig, func.__name__)
                for arg, arg_sig in zip(args, signature.parameters.values())
            ]
            kwargs = {
                key: cast_arg(val, signature.parameters[key], func.__name__)
