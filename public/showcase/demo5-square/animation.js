...
function updateCubes() {
    ...
    targets.forEach(cube => {
        ...
        let phase = theta / (2 * Math.PI);
        phase = (phase % 1) * 4;
        let radius = cube.range * WIDTH_SCALE;
        if (phase < 1) {
            cube.position.x = radius;
            cube.position.y = (2 * phase - 1) * radius;
        } else if (phase < 2) {
            cube.position.x = radius * (3 - 2 * phase)
            cube.position.y = radius;
        } else if (phase < 3) {
            cube.position.x = -radius;
            cube.position.y = radius * (5 - 2 * phase);
        } else {
            cube.position.x = radius * (2 * phase - 7);
            cube.position.y = -radius;
        }
    });
