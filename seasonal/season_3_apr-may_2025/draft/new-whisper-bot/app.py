...
class AppBase(ABC):
    ...
    @abstractmethod
    async def parse_with_llm(self, audio_pieces: list[Path]) -> list[str]:
        """Parse audio pieces using LLM"""
        pass

    @abstractmethod
    async def merge_transcription_pieces(self, transcription_pieces: list[str]) -> str:
    ...
    async def run(self, message_id: int, user_id: int) -> str:
        ...
        # Parse using llm
        transcription_pieces = await self.parse_with_llm(audio_pieces)

        # Merge pieces back
        full_transcription = await self.merge_transcription_pieces(transcription_pieces)
