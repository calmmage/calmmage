...
def _load_chats_from_json(filepath):
    ...
    for item in data['chats']:
        ...
        chat = {'entity': entity, 'last_message_date': datetime.fromisoformat(item['last_message_date']) if item['last_message_date'] else None}
        chats.append(chat)
...
async def _get_chats_from_client():
    ...
    for chat in chats:
        if type(chat.entity).__name__ == "ChatForbidden":
            ...
            continue
        result.append(
            {'entity': chat.entity, 'last_message_date': chat.date}
        )
    return result
...
def _has_fresh_chats_on_db(db, collection_name: str="chats", max_age: timedelta = timedelta(days=1)):
    collection = db[collection_name]
...
def calculate_stats(chats, big_threshold: int = 1000, recent_threshold: timedelta = timedelta(days=30)):
    """
    ...
    for chat in chats:
        ...
        recent = last_message_date and datetime.now(last_message_date.tzinfo) - last_message_date < recent_threshold
        if recent:
...
def is_recent(chat, recent_threshold: timedelta = timedelta(days=30)):
    last_message_date = chat['last_message_date']
    return last_message_date and datetime.now(last_message_date.tzinfo) - last_message_date < recent_threshold
