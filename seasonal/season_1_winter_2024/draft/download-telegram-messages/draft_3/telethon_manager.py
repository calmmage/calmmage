...
class TelethonClientManager:
    ...
    async def _create_new_telethon_client_and_save_to_disk(self, user_id: int) -> Optional[TelegramClient]:
    ...
    async def _get_telethon_client_from_database(self, user_id: int) -> Optional[TelegramClient]:
        ...
        if await self._check_if_conn_is_present_in_db(user_id):
            ...
            logger.debug(f"No existing connection found in database for user {user_id}, creating new")
            return await self._create_new_telethon_client_and_save_to_db(user_id)

    # 5 - create new conn from scratch -> save conn to db
    async def _create_new_telethon_client_and_save_to_db(self, user_id: int) -> Optional[TelegramClient]:
