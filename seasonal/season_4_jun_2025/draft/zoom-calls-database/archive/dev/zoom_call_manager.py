            ...
            if call.ai_transcript:
                ...
                try:
                    self._upload_file_to_airtable(
                        record_id, "AI Transcript", transcript_path
                    )
                finally:
...
""")
            ...
            if "AI Transcript" in attachment_fields:
                try:
                    ...
                    result = self._upload_file_to_airtable(
                        record_id, "AI Transcript", transcript_file
                    )
    ...
    def _get_s3_link(self, file_name: str) -> str:
        """Generate a public URL for the S3 object"""
        return f"https://{self.config.s3_bucket_name}.s3.{self.config.s3_region}.amazonaws.com/{file_name}"
    ...
    def _get_s3_presigned_url(
        self, object_key: str, expiration: int = 72 * 3600
    ) -> str:
        """Generate a presigned URL for S3 object access"""
        try:
            presigned_url = self.s3_conn.generate_presigned_url(
                "get_object",
                Params={"Bucket": self.config.s3_bucket_name, "Key": object_key},
                ExpiresIn=expiration,
            )
            print(
                f"Generated presigned URL for {object_key} (expires in {expiration} seconds)"
            )
    ...
    # def _make_s3_object_public(self, object_key: str) -> str:
    #     """Make an S3 object publicly readable and return public URL"""
    #     try:
    #         # Set object ACL to public-read
    #         self.s3_conn.put_object_acl(
    #             Bucket=self.config.s3_bucket_name,
    #             Key=object_key,
    #             ACL='public-read'
    #         )
    #         public_url = f"https://{self.config.s3_bucket_name}.s3.{self.config.s3_region}.amazonaws.com/{object_key}"
    #         print(f"Made {object_key} publicly accessible: {public_url}")
    #         return public_url
    #     except Exception as e:
    #         print(f"Failed to make {object_key} public: {e}")
    #         raise
